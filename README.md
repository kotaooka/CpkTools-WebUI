...
アップロードした Excel データから、解析対象列や上限・下限規格値を指定するだけで、最大値、最小値、平均値、標準偏差、Cp、Cpk、尖度、歪度などの統計量を瞬時に算出し、ヒストグラム、QQプロット、確率密度分布、X-bar管理図、I管理図、R管理図、MR管理図、s管理図などの多彩なグラフを自動生成します。
